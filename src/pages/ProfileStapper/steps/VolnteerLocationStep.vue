    ...
    <div class="scrollable-inputs q-px-md">
      ...
      <div v-if="showInputFields" class="input-fields">
        ...
        <div class="custom-input">
          ...
          >
            {{ t('common.selectFromMapLocation') }}
          </q-btn>
          <div
...
const updateLocationCoordinates = async (index: number) => {
  ...
  try {
    ...
    locations.value[index].location.coordinates = [
...
const removeLocation = async (index: number) => {
  try {
    ...
    const existingBusinessLocations =
...
const initializeLocations = () => {
  ...
  locations.value = JSON.parse(
    JSON.stringify(userLocations.filter((loc) => !loc.isBusinessLocation))
  );
  locationLoading.value = new Array(locations.value.length).fill(false);
};

const getCurrentLocationForNew = () => {
